...
| `repository/ci.rs` | CI/PR integration — `fetch_pr_info`, `fetch_ci_checks` (the GraphQL queries behind `gh pr list --head` / `gh pr checks`, and REST `check-runs` + `status` for branches without a PR) plus the pure, unit-tested mapping (`pr_info_from_node`, `aggregate_checks`, `summarize_checks`, `branch_ci_summary`). Both return `PrFetch`/`CiFetch` so callers can tell "no PR / no checks" from a rate-limit refusal; `fetch_ci_checks` skips the request entirely while the upstream commit still matches a settled cached result. `fetch_open_pull_requests` lists every open PR of a repository (all pages, drafts included) with each one's checks and readiness, returning `RepoPrsFetch`; anything short of the whole list is `Failed`, so the poller keeps the list it has. `list_pull_requests` (user action) still shells out to `gh`, passing `--repo HOST/OWNER/NAME` for a repository off github.com. |
| `repository/gh.rs` | Finding the `gh` binary without trusting the inherited `PATH` (the Dock's lacks Homebrew): Settings' `gh_path` (file or directory, installed process-wide by the daemon via `set_gh_path`), then `PATH`, then the usual install directories. Every gh spawn goes through `gh_command()`; never `command("gh")`. |
| `repository/github.rs` | GitHub API plumbing for the poll path — the known GitHub hosts (github.com, `*.ghe.com` tenants, gh's `hosts.yml` logins, `GH_HOST`, and Settings' enterprise hosts installed process-wide by the daemon via `set_enterprise_hosts`) that gate `has_github_remote` and base-repo resolution, base-repo resolution (`remote.<name>.gh-resolved`, then `origin` > `upstream` > `github` — not gh's order, so a fork's PRs, opened in `origin`, are the ones found) and `github_repo_key` (its lowercased `owner/name` on github.com, `host/owner/name` elsewhere, the key repository PR lists are shared under), gh's per-kind endpoints (`api.<host>` for github.com and GHE.com, `<host>/api/v3` for GHE Server), the process-wide token cache (the env vars gh reads for that kind of host, else one `gh auth token --hostname` spawn per 30 min), rate-limit detection, and `GithubClient` (REST with `Link` pagination + GraphQL) over `okena_transport::http`. |
