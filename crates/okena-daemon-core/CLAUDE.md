...
| `memory_poll.rs` | Resident memory of the daemon and of every terminal's process tree, per project (`ApiProcessMemory`). Published on a watch channel the stream attaches to each `SystemStatsChanged`. |
