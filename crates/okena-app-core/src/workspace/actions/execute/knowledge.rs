...
fn execute_at(
    ...
    Some(match action {
        ...
        ActionRequest::KnowledgeLayering => layering(registry, projects),
...
fn layer_order(registry: &Path, projects: &[ProjectSource]) -> Vec<KnowledgeRoot> {
    let all = discovered(registry, projects).roots;
...
/// nothing.
fn layering(registry: &Path, projects: &[ProjectSource]) -> ActionResult {
    let roots = layer_order(registry, projects);
